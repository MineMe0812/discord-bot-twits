...
module.exports = {
  ...
  async execute(message: Message) {
    ...
    setTimeout(async () => {
      ...
      // -> Format message
      const twitterMessageContent = formatMessageContentToTweet(
      ...
      const formattedMessageTwitter = twitterMessageContent.message;
      const isTwitterTicker = twitterMessageContent.isTwitterTicker;

      const stockTwitsMessageContent = formatMessageContentToTweet(
        updatedMessage.embeds.length &&
          updatedMessage.embeds[0].title &&
          updatedMessage.embeds[0].description
          ? `${updatedMessage.embeds[0].title}\n${updatedMessage.embeds[0].description}`
      ...
      const formattedMessageStockTwits = stockTwitsMessageContent.message;
      ...
      // -> Get hashtags
      const hashtags = getHashTags(channel.category, channel.hashtagCount, isTwitterTicker);
