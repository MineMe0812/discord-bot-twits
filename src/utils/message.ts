...
export const formatMessageContentToTweet = (
  ...
  let twitterTicker = false;                              // add by mineme
  ...
  const contentWithUpdatedTickers = contentWithoutMentions
    ...
    .replace(/(?<=\s|^|\/)[A-Z]+(?=\s|$|\/)/g, (match) => {
      ...
      if(futuresTickerExists) twitterTicker = true;
      ...
      if (isTwitter) {
        if (category) {
          ...
          return stockTickerExists || futuresTickerExists
          ? `$${match}`       
          : cryptoTickerExists || coinTickerExists
          ? `$${match}.X`
          : match;
        ...
        if (category) {
          return category === 'stocks' && stockTickerExists
          ?  (futuresTickerExists ? `$${match}_F` : `$${match}`)         
          : category === 'crypto' &&
            (cryptoTickerExists || coinTickerExists)
          ? `$${match}.X`
          : futuresTickerExists
      ...
      if (isTwitter && category)
        ...
        return stockTickerExists
        ? (futuresTickerExists && !isTwitter ? `$${match}_F` : `$${match}`)          
        : cryptoTickerExists || coinTickerExists
        ? `$${match}.X`
        : futuresTickerExists
        ? `$${match}_F`
    ...
    .replace(/(\*|)/g, '');                                              //changed by mineme
    //.replace(/(\*|_)/g, ''); 
    
    return {"message":contentWithUpdatedTickers, "isTwitterTicker": twitterTicker};
};
